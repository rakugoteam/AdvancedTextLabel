...
func _open_file(file:String) -> void:
	var new_text_view = text_view_scene.instantiate()
	new_text_view.markup_text_file = file
	new_text_view.connect("meta_clicked", Callable(self, "_on_meta_clicked"))
	tabs.add_child(new_text_view)
	tabs.current_tab = tabs.get_tab_count() - 1

	var new_tab_button = tab_button_scene.instantiate()
	...
	button.connect("pressed", Callable(self, "_on_tab_pressed").bind(new_tab_button))

	var close_button : Button = new_tab_button.get_node("CloseButton")
	close_button.connect("pressed", Callable(self, "_on_tab_closed").bind(new_tab_button))
